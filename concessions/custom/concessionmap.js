...
DATADIR='../data/';
HEIGHTMARGIN= 10; // how much space to leave below the map

var map = L.map('map', {
    fullscreenControl: true
}).setView([4.22, 18.35], 3);
$(window).on("resize", function() {
    $("#map").height($(window).height() - HEIGHTMARGIN);
    map.invalidateSize();
}).trigger("resize");
...
filenames = typeof(CUSTOM_FILENAMES) === 'undefined' ? all_filenames: CUSTOM_FILENAMES; // allow to override from outside, to select a particular country
...
var choose_layers = function(){
	...
	queryDict['c'].toUpperCase().split(',').forEach(
		...
		if(n in all_countries){
		    filepaths.push(DATADIR + all_countries[n]);
		}
	    });
    }
    else{
	for(var key in all_countries){
	    filepaths.push(DATADIR + all_countries[key]);
	}
    }
    return filepaths;
}
...
var mapboxAttribution = 'Block data &copy <a href="http://maps.openoil.net/concessions">OpenOil</a> Map base data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors, both <a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, Imagery © <a href="http://mapbox.com">Mapbox</a>'
